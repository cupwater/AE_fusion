...
class RestormerEncoder(nn.Module):
    ...
    def forward(self, vis_img, ir_img):
        ...
        vis_base_feat = self.base_layer(vis_feat_shared)
        ir_base_feat  = self.base_layer(ir_feat_shared)
        vis_detail_feat = self.detail_layer(vis_feat_shared)
        ir_detail_feat  = self.detail_layer(ir_feat_shared)
