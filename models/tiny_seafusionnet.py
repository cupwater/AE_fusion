...
class TinySeAFusionNet(nn.Module):
    ...
    def forward(self, rgb, ir):
        ...
        fused_img = torch.transpose(fused_img, 3, 1)
        fused_img = torch.transpose(fused_img, 2, 1)
        return fused_img
...
if __name__ == '__main__':
    ...
    torch.onnx.export(pt_model,               # model being run
                    (rgb_tensor, ir_tensor),                         # model input (or a tuple for multiple inputs)
                    "fusion_data/LightSeAFusionRaw.onnx",   # where to save the model (can be a file or file-like object)
                    opset_version=11,          # the ONNX version to export the model to
                    input_names = ['rgb', 'ir'],   # the model's input names
