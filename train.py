...
scheduler = MultiStepLR(optimizer, [epochs//3, epochs//3*2], gamma=0.1)
...
for iteration in range(epochs):
    ...
    for step in range(Iter_per_epoch):
        ...
        out_vis, vis_feat_bg, vis_feat_detail, out_ir, \
                ir_feat_bg, ir_feat_detail = model(data_VIS, data_IR)

        mse_loss_B = L1Loss(ir_feat_bg, vis_feat_bg)
        mse_loss_D = L1Loss(ir_feat_detail, vis_feat_detail)

        import pdb
        # pdb.set_trace()

        mse_loss_VF = 5*ssim(data_VIS, out_vis) + MSELoss(data_VIS, out_vis)
        mse_loss_IF = 5*ssim(data_IR, out_ir) + MSELoss(data_IR,  out_ir)

        Gradient_loss = L1Loss(
            kornia.filters.SpatialGradient()(data_VIS),
            kornia.filters.SpatialGradient()(out_vis)
        )
        # Total loss
        ...
        print('Epoch/step: %d/%d, loss: %.7f, lr: %f' % (iteration+1,
              step+1, los, optimizer.state_dict()['param_groups'][0]['lr']))
...
torch.save( {'weight': model.state_dict(), 'epoch':epochs}, 
