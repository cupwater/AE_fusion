...
Iter_per_epoch = (Train_Image_Number % batch_size != 0) + \
    Train_Image_Number//batch_size
# =============================================================================
...
dataloader_VIS = torch.utils.data.DataLoader(
    Data_VIS, batch_size, shuffle=False)
...
scheduler1 = torch.optim.lr_scheduler.MultiStepLR(
    optimizer1, [epochs//3, epochs//3*2], gamma=0.1)
scheduler2 = torch.optim.lr_scheduler.MultiStepLR(
    optimizer2, [epochs//3, epochs//3*2], gamma=0.1)
...
for iteration in range(epochs):
    ...
    for step in range(Iter_per_epoch):
        ...
        # Calculate loss
        # =====================================================================
        feature_V_1, feature_V_2, feature_V_B, feature_V_D = AE_Encoder(
            data_VIS)
        feature_I_1, feature_I_2, feature_I_B, feature_I_D = AE_Encoder(
            data_IR)
        img_recon_V = AE_Decoder(
            feature_V_1, feature_V_2, feature_V_B, feature_V_D)
        img_recon_I = AE_Decoder(
            feature_I_1, feature_I_2, feature_I_B, feature_I_D)

        mse_loss_B = L1Loss(feature_I_B, feature_V_B)
        mse_loss_D = L1Loss(feature_I_D, feature_V_D)

        mse_loss_VF = 5*ssim(data_VIS, img_recon_V) + \
            MSELoss(data_VIS, img_recon_V)
        mse_loss_IF = 5*ssim(data_IR,  img_recon_I) + \
            MSELoss(data_IR,  img_recon_I)
        ...
        # Total loss
        loss = 2*mse_loss_VF + 2*mse_loss_IF + \
            torch.tanh(mse_loss_B) - 0.5 * \
            torch.tanh(mse_loss_D) + 10*Gradient_loss
        ...
        print('Epoch/step: %d/%d, loss: %.7f, lr: %f' % (iteration+1,
              step+1, los, optimizer1.state_dict()['param_groups'][0]['lr']))
...
plt.subplot(2, 3, 2), plt.plot(Average_loss(
    mse_loss_B_train)), plt.title('Base_layer_loss')
plt.subplot(2, 3, 3), plt.plot(Average_loss(
    mse_loss_D_train)), plt.title('Detail_layer_loss')
plt.subplot(2, 3, 4), plt.plot(Average_loss(
    mse_loss_VF_train)), plt.title('V_recon_loss')
plt.subplot(2, 3, 5), plt.plot(Average_loss(
    mse_loss_IF_train)), plt.title('I_recon_loss')
plt.subplot(2, 3, 6), plt.plot(Average_loss(
    Gradient_loss_train)), plt.title('Gradient_loss')
plt.tight_layout()
