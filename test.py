...
LastEditTime: 2023-02-01 14:58:58
Description: 
...
for i in range(int(Test_Image_Number/2)):
    if Test_data_choose == 'Test_data_TNO':
        Test_IR = Image.open(test_data_path+'/IR' +
                             str(i+1)+'.bmp')  # infrared image
        Test_Vis = Image.open(test_data_path+'/VIS' +
                              str(i+1)+'.bmp')  # visible image
    elif Test_data_choose == 'Test_data_NIR_Country':
        Test_IR = Image.open(test_data_path+'/IR' +
                             str(i+1)+'.png')  # infrared image
        Test_Vis = Image.open(test_data_path+'/VIS' +
                              str(i+1)+'.png')  # visible image
    elif Test_data_choose == 'Test_data_FLIR':
        Test_IR = Image.open(test_data_path+'/IR' +
                             str(i+1)+'.jpg')  # infrared image
        Test_Vis = Image.open(test_data_path+'/VIS' +
                              str(i+1)+'.jpg')  # visible image
