...
if __name__ == '__main__':
    ...
    BaseFuseLayer = nn.DataParallel(BaseFeatureExtraction(dim=64, num_heads=8))
    DetailFuseLayer = nn.DataParallel(DetailFeatureExtraction(num_layers=1))
    ...
    for i in range(30):
        feature_V_B, feature_V_D, feature_V = encoder(vis_img)
        feature_I_B, feature_I_D, feature_I = encoder(ir_img)
        feature_F_B = BaseFuseLayer(feature_I_B+feature_V_B)
        feature_F_D = DetailFuseLayer(feature_I_D+feature_V_D)
        data_Fuse, feature_F = decoder(vis_img, feature_F_B, feature_F_D)  
